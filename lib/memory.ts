...
export class MemoryManager {
    ...
    public async init() {
        if (this.vectorDBClient instanceof PineconeClient) await this.vectorDBClient.init({
    ...
    public async writeToHistory(text: string, companionKey: CompanionKey) {
        if (!companionKey || typeof companionKey.userId === "undefined") {
            console.log("Companion key set incorrectly");// dev log
            return "";
        }
    ...
    public async seedChatHistory(
        ...
        if (await this.history.exists(key)) {
            console.log("User already has chat history");// dev log
            return;
        }
